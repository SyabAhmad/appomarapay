...
const DetailedReceipt: React.FC<Props> = ({ navigation, route }) => {
  ...
  const checkGoogleWalletStatus = async () => {
    ...
    try {
      ...
      if (['succeeded','paid','completed'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [{ name: 'CardSuccess' as never, params: { success: true, chainName: 'Google Wallet', tokenSymbol: 'USD', tokenAmount, usdAmount, mobile, receivingAddress } as never }],
        });
      } else if (['failed','canceled'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [{ name: 'CardFailure' as never, params: { chainName: 'Google Wallet', tokenSymbol: 'USD', usdAmount, mobile, errorMessage: `Payment ${status}` } as never }],
        });
      } else {
        Alert.alert('Status', `Payment status: ${status || 'pending'}`);
      }
  ...
  const onDone = () => {
    navigation.reset({ index: 0, routes: [{ name: 'CardFailure' as never, params: { errorMessage: 'Payment not completed' } as never }] });
  };
