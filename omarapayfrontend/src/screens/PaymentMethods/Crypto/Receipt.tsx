...
type Props = NativeStackScreenProps<RootStackParamList, 'CryptoReceipt'>;
...
const DetailedReceipt: React.FC<Props> = ({ navigation, route }) => {
  ...
  const checkCryptoStatus = async () => {
    ...
    try {
      ...
      if (verified || ['COMPLETED', 'RESOLVED', 'CONFIRMED'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [
            {
      ...
      if (['EXPIRED', 'CANCELED', 'FAILED', 'UNRESOLVED'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [
            {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.headerRow}>
        <Text style={styles.header}>Crypto Payment</Text>
      ...
      <View style={styles.qrWrap}>
        ...
        {hosted_url ? (
          ...
          <Image
            source={{ uri: `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(receivingAddress)}` }}
            style={styles.qrImage}
            resizeMode="contain"
          />
        ) : (
          <Text style={{ color: '#6b7280', marginBottom: 12 }}>No checkout link yet</Text>
        )}
        ...
        <TouchableOpacity
          ...
          onPress={checkCryptoStatus}
          disabled={checking}
        >
          {checking ? <ActivityIndicator color="#fff" /> : <Text style={styles.primaryBtnText}>Check crypto status</Text>}
        </TouchableOpacity>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.amount}>${Number(usdAmount || '0').toFixed(2)}</Text>
