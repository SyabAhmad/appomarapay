  ...
  const checkGoogleWalletStatus = async () => {
    ...
    try {
      ...
      if (['succeeded','paid','completed'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [{ name: 'CryptoSuccess' as never, params: { success: true, chainName: 'Google Wallet', tokenSymbol: 'USD', tokenAmount, usdAmount, mobile, receivingAddress } as never }],
        });
      } else if (['failed','canceled'].includes(status)) {
        navigation.reset({
          index: 0,
          routes: [{ name: 'CryptoFailure' as never, params: { chainName: 'Google Wallet', tokenSymbol: 'USD', usdAmount, mobile, errorMessage: `Payment ${status}` } as never }],
        });
      } else {
        Alert.alert('Status', `Payment status: ${status || 'pending'}`);
      }
        ...
        onPress={() =>
          navigation.reset({
            ...
            routes: [
              {
                name: 'CryptoFailure' as never,
                params: { errorMessage: 'Payment not completed' } as never,
              },
            ],
          })
        }
