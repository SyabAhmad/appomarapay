...
// Google Wallet OTP verification only (no other payment types)
type RootStackParamList = {
  ...
  GoogleWalletPay: { amount: string; currency?: string; description?: string; metadata?: Record<string, any>; googleWallet?: boolean } | undefined;
};

type Props = NativeStackScreenProps<RootStackParamList, 'GoogleWalletOtp'>;
...
const OtpVerification: React.FC<Props> = ({ navigation, route }) => {
  ...
  const onVerify = async () => {
    ...
    // Google Wallet uses Stripe PaymentSheet with Google Pay
    navigation.reset({
      index: 0,
      routes: [
        {
          name: 'GoogleWalletPay' as never,
          params: {
            amount: String(Number(selectedAmount || '0').toFixed(2)),
            currency: 'usd',
            description: `Payment for ${chainName ?? 'Google Wallet'}`,
            metadata: { phone, chainName: chainName ?? 'Google Wallet', tokenSymbol: 'USD', channel: 'googlewallet' },
            googleWallet: true,
          } as never,
        },
      ],
