...
import { SafeAreaView, View, Text, TouchableOpacity, Image, StyleSheet, Platform, StatusBar } from 'react-native';
...
type Props = NativeStackScreenProps<RootStackParamList, 'GCashPhone'>;

const PhoneConfirmation: React.FC<Props> = ({ navigation, route }) => {
  const { chainId, chainName, selectedAmount } = route.params ?? {};
  ...
  const sendOtp = () => {
    ...
    setTimeout(() => {
      setSending(false);
      navigation.navigate('GCashOtp' as never, {
        chainId,
        chainName,
        selectedAmount,
        phone: `+${digits}`,
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.summary}>
        <Text style={styles.summaryLine}>{chainName ?? 'GCash'}</Text>
        <Text style={styles.amountText}>₱{Number(selectedAmount || '0').toFixed(2)}</Text>
      </View>
...
const styles = StyleSheet.create({
  ...
  phoneDisplay: { backgroundColor: '#f1f5f9', width: '100%', borderRadius: 10, paddingVertical: 16, alignItems: 'center', justifyContent: 'center' },
  phoneText: { fontSize: 24, fontWeight: '800', color: '#0f172a' },
  hint: { color: '#94a3b8', marginTop: 10, textAlign: 'center', fontSize: 13 },
  keyboardWrap: { width: '100%', maxWidth: 560, marginTop: 12, paddingHorizontal: 16, paddingBottom: 140 },
  footer: { position: 'absolute', left: 16, right: 16, bottom: 18, alignItems: 'center' },
  sendBtn: { width: '100%', maxWidth: 560, backgroundColor: '#2563eb', paddingVertical: 14, borderRadius: 12, alignItems: 'center' },
